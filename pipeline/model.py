...
class LAB(nn.Module):
    def __init__(self, dim):
        ...
        self.conv_res1 = nn.Sequential(
            nn.Conv2d(self.dim, self.dim, 3, 1, 1, bias=False, groups=self.dim),
            nn.ReLU(True),
            nn.Conv2d(self.dim, self.dim, 3, 1, 1, bias=False, groups=self.dim),
        )
    ...
    def forward(self, x):
        ...
        x1 = self.conv_res1(x) + x
        x2 = self.conv_res2(x1) + x1
        x3 = self.conv_res3(x2) + x2
        x4 = self.conv_res4(x3) + x3
...
class AFFB(nn.Module):
    ...
    def forward(self, x1, x2):
        feas, fea = x1.unsqueeze_(dim=1), x2.unsqueeze_(dim=1)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.channel_attn = CAB(self.dim)
        self.conv_res = LAB(self.dim)
        self.fusion = AFFB(self.dim)
    ...
    def forward(self, x):
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                x_trans = rearrange(x_trans, 'b (h w) c -> b c h w', h=int(x.shape[1] ** 0.5))

                x_conv = self.conv_res(x_ca)
                x_fusion = self.fusion(x_trans, x_conv)

                x = rearrange(x_fusion, 'b c h w -> b (h w) c')
...
class UDAFT(nn.Module):
    def __init__(self):
        super(UDAFT, self).__init__()
        ...
        for i_layer in range(self.num_layers):  # len([8, 8, 8, 8]) = 4
